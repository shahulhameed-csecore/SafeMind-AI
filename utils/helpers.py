...
5. Keep your replies concise and conversational."""
    ...
    english_reply = ""
    max_retries = 3 # 👈 Tell the app to try up to 3 times
    ...
    for attempt in range(max_retries):
        try:
            ...
            error_str = str(e)
            # If Google throws a 500 (Internal) or 503 (Overloaded) error, wait 2 seconds and try again
            if "500" in error_str or "503" in error_str:
                print(f"⚠️ Google Server Hiccup (Attempt {attempt + 1}/{max_retries}). Retrying in 2 seconds...")
                time.sleep(2)
            else:
                # If it's a different error, fail gracefully and ask the user to repeat
                print(f"❌ API Error: {e}")
                english_reply = "I am taking a deep breath right now. Could you please share that with me again?"
                break
    else:
        # If it fails 3 times in a row, let the user know
