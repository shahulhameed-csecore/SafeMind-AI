...
def generate_ai_response(user_input, chat_history, user_lang='en'):
    ...
    # 🧠 THE ML FLEX: FEW-SHOT CLINICAL PROMPTING
...
SafeMind: "It sounds like you're carrying a heavy weight right now. Failing an exam hurts, but one test does not define your entire worth. Let's take a breath together. What is one small thing you can do for yourself today?"

User: "My chest is tight and I can't stop worrying about the future."
SafeMind: "That tightness sounds really overwhelming. I hear you. When anxiety pulls us into the future, grounding ourselves in the present can help. Would you like to try a quick breathing exercise with me?"
"""
    ...
    for attempt in range(max_retries):
        try:
            ...
            error_str = str(e)
            if "429" in error_str or "500" in error_str or "503" in error_str or "timeout" in error_str.lower():
                wait_time = base_wait_time * (2 ** attempt)
                print(f"⚠️ API Limit Hit (Attempt {attempt + 1}/{max_retries}). Applying Exponential Backoff. Retrying in {wait_time}s...")
        ...
        english_reply = "The servers are taking a moment to process. Please take a deep breath and try sending that again."
