...
# 🌟 PRIMARY BRAIN (OpenRouter Gemma)
try:
    gemma_primary = OpenAI(
        base_url="https://openrouter.ai/api/v1",
        api_key=os.getenv("OPENROUTER_API_KEY") 
    )
    print("✅ Primary AI (OpenRouter) Initialized")
except Exception as e:
    print(f"⚠️ Failed to initialize OpenRouter: {e}")
    gemma_primary = None

# 🌟 BACKUP BRAIN (Groq Gemma)
try:
    gemma_backup = Groq(api_key=os.getenv("GROQ_API_KEY"))
    print("✅ Backup AI (Groq) Initialized")
except Exception as e:
...
def generate_ai_response(user_input, chat_history, user_lang='en'):
    ...
    # 2. Translate Chat History to English Firewall
    english_history = []
    ...
    # 🚀 PLAN A: Try OpenRouter (Gemma)
    try:
        if not gemma_primary: raise ValueError("Primary client offline.")
        
        chat_completion = gemma_primary.chat.completions.create(
        ...
        print("✅ Response generated by: GEMMA (Plan A - OpenRouter)")

    except Exception as primary_error:
        print(f"⚠️ OpenRouter Failed ({primary_error}). Initiating Groq Failover...")
        
        # 🚀 PLAN B: OpenRouter failed! Fallback to Groq (Gemma)
        try:
            if not gemma_backup: raise ValueError("Backup client offline.")
            
            chat_completion = gemma_backup.chat.completions.create(
                model="gemma2-9b-it", 
                messages=[
                    {"role": "system", "content": system_prompt},
                    {"role": "user", "content": enforced_input}
            ...
            english_reply = chat_completion.choices[0].message.content
            print("✅ Response generated by: GEMMA (Plan B - Groq)")
            
        except Exception as backup_error:
            # 💥 PLAN C: Both providers are down.
            print(f"❌ Both APIs failed: {backup_error}")
    ...
    if user_lang != 'en' and "connection issue" not in english_reply:
        try:
            # Safely translates the pure English AI output back into the exact language requested by the dropdown
