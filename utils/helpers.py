...
def generate_ai_response(user_input, chat_history, user_lang='en'):
    ...
    # 🚀 PLAN A: Try the highly stable Gemma 2 27B model
    try:
        if not gemma_client: raise ValueError("Client offline.")
        
        chat_completion = gemma_client.chat.completions.create(
            model="google/gemma-2-27b-it:free", # 🚀 FIXED: Reliable OpenRouter ID
            messages=[
        ...
        print("✅ Response generated by: GEMMA 2 27B (Plan A)")

    except Exception as primary_error:
        print(f"⚠️ Plan A Failed ({primary_error}). Initiating Plan B Failover...")
        
        # 🚀 PLAN B: Fallback to the ultra-fast Gemma 2 9B model
        try:
            if not gemma_client: raise ValueError("Client offline.")
            
            chat_completion = gemma_client.chat.completions.create(
                model="google/gemma-2-9b-it:free", # 🚀 FIXED: Lightning fast fallback
                messages=[
                    {"role": "system", "content": system_prompt},
                    {"role": "user", "content": enforced_input}
                ],
            ...
            english_reply = chat_completion.choices[0].message.content
            print("✅ Response generated by: GEMMA 2 9B (Plan B)")
