...
try:
    ...
    print("✅ Gemma API Client Initialized via OpenRouter")
except Exception as e:
    print(f"⚠️ Failed to initialize Gemma API: {e}")
    gemma_client = None
...
def generate_ai_response(user_input, chat_history, user_lang='en'):
    ...
    # 🚨 COMPETITION SPECIFIC: Using Google Gemma via OpenRouter
    try:
        if not gemma_client: raise ValueError("Gemma client offline.")
        
        chat_completion = gemma_client.chat.completions.create(
            # Using the 9B model. (You can switch this string to "google/gemma-4-9b-it" if OpenRouter has updated their tags for the newest version)
        ...
        print("✅ Response generated by: GEMMA (via OpenRouter)")

    except Exception as api_error:
        print(f"❌ Gemma API failed: {api_error}")
