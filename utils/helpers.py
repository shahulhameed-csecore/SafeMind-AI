...
# 🌟 100% PURE GEMMA 4 (Kaggle Compliant & 100% Free)
try:
    gemma_client = OpenAI(
        base_url="https://openrouter.ai/api/v1",
        api_key=os.getenv("OPENROUTER_API_KEY") 
    )
    print("✅ Pure Gemma 4 AI Initialized")
...
def generate_ai_response(user_input, chat_history, user_lang='en'):
    ...
    # 🚀 PLAN A: Free Gemma 4 MoE model
 # 🚀 PLAN A: Free Gemma 4 model (Lightweight & Fast)
    try:
        if not gemma_client: raise ValueError("Client offline.")
        
        chat_completion = gemma_client.chat.completions.create(
            model="google/gemma-4-9b-it:free", # 👈 CHANGE THIS LINE RIGHT HERE
            messages=[
        ...
        english_reply = chat_completion.choices[0].message.content.replace('*', '').strip()
        print("✅ Response generated by: FREE GEMMA 4")

    except Exception as e:
        print(f"❌ Rate Limit Hit: {e}")
        # Graceful fallback so the server doesn't crash during the hackathon
        english_reply = "The network is a bit crowded right now. Could you please send that again in a few seconds?"
         
    if user_lang != 'en' and "crowded" not in english_reply:
        try:
